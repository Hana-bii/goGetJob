...
import { resolveWebSocketUrl } from '../api/runtime';
...
export default function VoiceInterviewPage() {
  ...
  // Cleanup on unmount 鈥?鑷姩鏆傚仠鏈粨鏉熺殑 session
  useEffect(() => {
    return () => {
      if (timerRef.current) {
      ...
      clearPendingAiTextCommit();
      // 鐢ㄦ埛娌℃湁涓诲姩缁撴潫/鏆傚仠鏃讹紝鑷姩鏆傚仠 session
  ...
  useEffect(() => {
    if (aiAudio && audioPlayerRef.current) {
      ...
      if (playPromise !== undefined) {
        playPromise.catch(() => {
          setError('璇风偣鍑婚〉闈换鎰忎綅缃互鍚敤闊抽鎾斁');
          setAiSpeaking(false);
          setIsSubmitting(false);
        });
      }
  ...
  const createWebSocketHandlers = useCallback(() => ({
    ...
    onSubtitle: (text: string, isFinal: boolean) => {
      // 鎵嬪姩鎻愪氦妯″紡涓嬶紝isFinal=true 鐢?triggerLlmResponse 瑙﹀彂锛堟彁浜ょ敤鎴锋秷鎭埌鍘嗗彶锛?
    ...
    onClose: (event: { code: number }) => {
      ...
      clearPendingAiTextCommit();
      if (event.code !== 1000) {
        setError('杩炴帴宸叉柇寮€锛岃鍒锋柊椤甸潰閲嶈瘯');
      }
    },
    onError: () => {
      clearPendingAiTextCommit();
      setError('WebSocket 杩炴帴閿欒锛岃妫€鏌ョ綉缁滃悗閲嶈瘯');
      setConnectionStatus('disconnected');
    },
  ...
  const connectWithHandlers = useCallback((sessionId: number, wsUrl: string) => {
    ...
    connectTimeoutRef.current = setTimeout(() => {
      try {
        ...
        setError('鏃犳硶寤虹珛 WebSocket 杩炴帴: ' + (error instanceof Error ? error.message : '鏈煡閿欒'));
        setConnectionStatus('disconnected');
      }
    }, 500);
  ...
  const handlePhaseConfig = useCallback(async (config: {
    ...
    try {
      ...
      const wsUrl = resolveWebSocketUrl(session.webSocketUrl, `/ws/voice-interview/${session.sessionId}`);
      connectWithHandlers(session.sessionId, wsUrl);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : '鍒涘缓闈㈣瘯浼氳瘽澶辫触锛岃閲嶈瘯';
  ...
  const handleResumeSession = useCallback(async (id: number) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : '鎭㈠浼氳瘽澶辫触');
      setConnectionStatus('disconnected');
    }
  }, [connectWithHandlers]);
  ...
  // 楹﹀厠椋庨煶棰戞寔缁彂閫佺粰鏈嶅姟绔仛 ASR锛堟墜鍔ㄦ彁浜ゆā寮忎笅涓嶉渶瑕?blockade锛屽洖澹颁笉浼氳Е鍙?LLM锛?
  const handleAudioData = (audioData: string) => {
    if (wsRef.current && wsRef.current.isConnected()) {
      wsRef.current.sendAudio(audioData);
    } else {
      setError('Not connected to the server, please refresh and try again');
    }
  };
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center p-6">
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm p-8 text-center max-w-md w-full">
          ...
          <p className="text-slate-700 dark:text-slate-200 text-lg font-semibold mb-2">鏈娴嬪埌璇煶闈㈣瘯閰嶇疆</p>
          <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">Please start from the interview history or the voice interview entry.</p>
          <button
  ...
  return (
    <div className="pb-10">
      <div className="max-w-7xl mx-auto">
        <InterviewPageHeader
          title="璇煶妯℃嫙闈㈣瘯"
          subtitle="Real-time voice interview with live follow-up questions"
          ...
          <div className="xl:col-span-2 space-y-6">
                ...
                <div className="w-full max-w-2xl min-h-[130px] rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-700 px-6 py-5 text-center flex items-center justify-center">
                  <AnimatePresence mode="wait">
                    {isAiSpeaking || aiText ? (
                      ...
                      >
                        {isRecording ? '姝ｅ湪鑱嗗惉锛岃瀹屽悗鐐瑰嚮"鎻愪氦鍥炵瓟"...' : '鐐瑰嚮楹﹀厠椋庡紑濮嬪彂瑷€'}
                      </motion.p>
                    )}
                  </AnimatePresence>
                </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm p-5">
              <div className="flex items-center justify-center gap-6">
                <button
                  onClick={() => {
                    const choice = window.confirm('鏆傚仠闈㈣瘯锛焅n纭畾 = 鐭殏鍋滐紙5鍒嗛挓锛塡n鍙栨秷 = 绂诲紑骞朵繚瀛?');
              ...
              <p className="text-center text-xs text-slate-500 dark:text-slate-400 mt-3">
                {isAiSpeaking ? '闈㈣瘯瀹樻鍦ㄥ洖绛?..' : isSubmitting ? '姝ｅ湪鎬濊€?..' : isRecording ? '璇村畬鍚庣偣鍑?鎻愪氦鍥炵瓟"' : '鐐瑰嚮楹﹀厠椋庡彂瑷€'}
              </p>
            </div>
          </div>
